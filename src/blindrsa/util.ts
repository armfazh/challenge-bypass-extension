...
function getRandomSalt(sLen: number): Uint8Array {
    return crypto.getRandomValues(new Uint8Array(sLen));
}

// Exported so tests can mock this function.
export const mockableFn = { getRandomSalt };
...
export async function emsa_pss_encode(
    ...
    const salt = mockableFn.getRandomSalt(sLen);
    // 5.  Let
